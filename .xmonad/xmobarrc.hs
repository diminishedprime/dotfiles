...
Config {
  ...
  commands = [
    ...
    -- Battery information. This is likely to require some customization based
    -- upon your specific hardware. Or, for a desktop you may want to just
    -- remove this section entirely.
    Run Battery [ "-t", "B:<left>"
                , "--"
                , "-p", "orange"
                , "-O", "AC"
                , "-i", "AC"
   ...
   template = "%StdinReader% }{ %mybacklight% %battery% %cpu% %memory% V:%myvolume% <fc=#e6744c>%date%</fc>     "
}
